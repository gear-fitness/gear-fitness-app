...
public class WorkoutController {
  ...
  public ResponseEntity<List<WorkoutDTO>> getWorkoutsByUser(@PathVariable UUID userId) {
    try {
      List<WorkoutDTO> workouts =
          workoutService.getWorkoutsByUser(userId).stream()
              .map(
                  w ->
                      new WorkoutDTO(
                          w.getWorkoutId(), w.getName(), w.getDatePerformed(), w.getCreatedAt()))
  ...
  public ResponseEntity<List<WeeklyVolumeDTO>> getWeeklyVolume(
      @PathVariable UUID userId, @RequestParam(defaultValue = "12") int weeks) {
    try {
  ...
  public ResponseEntity<Void> deleteWorkout(
      @PathVariable UUID workoutId, @RequestHeader("Authorization") String authHeader) {
    try {
      String token = authHeader.substring(7); // Remove "Bearer "
      UUID userId = jwtService.extractUserId(token);
