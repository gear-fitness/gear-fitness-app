...
public class ExerciseChatController {
  ...
  public ResponseEntity<ChatResponseDTO> chat(
      @PathVariable UUID exerciseId, @RequestBody ChatRequestDTO request) {

    // --- CRITICAL FIX START ---
    List messages = request.getMessages();

    // 1. Check if the DTO payload is missing the messages list (null) or if the
    // list is empty
    if (messages == null || messages.isEmpty()) {
      ...
      return ResponseEntity.badRequest()
          .body(
              new ChatResponseDTO(
                  "Conversation history is missing or empty. Please ensure your request body is"
