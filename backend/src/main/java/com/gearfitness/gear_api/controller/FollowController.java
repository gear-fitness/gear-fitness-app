...
public class FollowController {
  ...
  public ResponseEntity<?> followUser(
    ...
    try {
      ...
      return ResponseEntity.badRequest()
          .body(FollowResponse.builder().message(e.getMessage()).build());
    } catch (Exception e) {
      return ResponseEntity.internalServerError().body("An error occurred");
    }
  }

  /** POST /api/follows/username/{username} Follow a user by their username */
  @PostMapping("/username/{username}")
  public ResponseEntity<?> followUserByUsername(
      @RequestHeader("Authorization") String authHeader, @PathVariable String username) {
    try {
      String token = authHeader.substring(7);
      UUID followerId = jwtService.extractUserId(token);
  ...
  /** GET /api/follows/{userId}/followers Get list of users following the specified user */
  ...
  /** GET /api/follows/{userId}/following Get list of users that the specified user is following */
  ...
  /** GET /api/follows/{userId}/status Check if current user is following the specified user */
  ...
  /** GET /api/follows/requests Get pending follow requests for current user */
  @GetMapping("/requests")
  public ResponseEntity<?> getPendingRequests(@RequestHeader("Authorization") String authHeader) {
    try {
      ...
      return ResponseEntity.internalServerError().body("Failed to fetch follow requests");
    }
  }

  /** POST /api/follows/requests/{followerId}/accept Accept a follow request */
  @PostMapping("/requests/{followerId}/accept")
  public ResponseEntity<?> acceptFollowRequest(
      @RequestHeader("Authorization") String authHeader, @PathVariable UUID followerId) {
    try {
      String token = authHeader.substring(7);
      ...
      return ResponseEntity.internalServerError().body("Failed to accept follow request");
    }
  }

  /** DELETE /api/follows/requests/{followerId} Decline a follow request */
  ...
  public ResponseEntity<?> declineFollowRequest(
    ...
    try {
      ...
      return ResponseEntity.internalServerError().body("Failed to decline follow request");
    }
  }

  /** GET /api/follows/activity Get recent follow activity for current user */
  @GetMapping("/activity")
  public ResponseEntity<?> getFollowActivity(@RequestHeader("Authorization") String authHeader) {
    try {
      String token = authHeader.substring(7);
      UUID userId = jwtService.extractUserId(token);
      ...
      return ResponseEntity.internalServerError().body("Failed to fetch follow activity");
    }
  }
}
