...
public class FollowService {
  ...
  /** Follow a user Creates a follow relationship with ACCEPTED or PENDING status */
  @Transactional
  public FollowResponse followUser(UUID followerId, UUID followeeId) {
    ...
    Follow.FollowStatus status =
        followee.getIsPrivate() ? Follow.FollowStatus.PENDING : Follow.FollowStatus.ACCEPTED;
  ...
  public List<FollowerDTO> getFollowers(UUID userId) {
    ...
    return followRepository.findByFolloweeAndStatus(user, Follow.FollowStatus.ACCEPTED).stream()
        .map(f -> new FollowerDTO(f.getFollower().getUserId(), f.getFollower().getUsername()))
  ...
  public List<FollowerDTO> getFollowing(UUID userId) {
    ...
    return followRepository.findByFollowerAndStatus(user, Follow.FollowStatus.ACCEPTED).stream()
        .map(f -> new FollowerDTO(f.getFollowee().getUserId(), f.getFollowee().getUsername()))
        .collect(Collectors.toList());
  }
  ...
  public List<FollowerDTO> getPendingRequests(UUID userId) {
    ...
    return followRepository.findByFolloweeAndStatus(user, Follow.FollowStatus.PENDING).stream()
        .map(f -> new FollowerDTO(f.getFollower().getUserId(), f.getFollower().getUsername()))
        .collect(Collectors.toList());
  }
        ...
        .map(
            f ->
                new FollowActivityDTO(
                    f.getFollower().getUserId(), f.getFollower().getUsername(), f.getCreatedAt()))
        .collect(Collectors.toList());
  }
}
