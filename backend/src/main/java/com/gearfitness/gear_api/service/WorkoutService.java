...
public class WorkoutService {
        ...
        for (WorkoutExercise exercise : workout.getWorkoutExercises()) {
          for (WorkoutSet set : exercise.getWorkoutSets()) {
            if (set.getWeightLbs() != null && set.getReps() != null) {
              BigDecimal setVolume = set.getWeightLbs().multiply(new BigDecimal(set.getReps()));
  ...
  public List<DailyVolumeDTO> getDailyVolume(
      UUID userId, int numberOfWeeks, DayOfWeek weekStartDay) {
    List<Workout> workouts = workoutRepository.findByUser_UserId(userId);
    ...
    if (numberOfWeeks <= 0) {
      ...
      startDate =
          endDate.minusWeeks(numberOfWeeks).with(TemporalAdjusters.previousOrSame(weekStartDay));
    }
  ...
  @Transactional
  public WorkoutDetailDTO submitWorkout(WorkoutSubmissionDTO submission, UUID userId) {
    ...
    Workout workout =
        Workout.builder()
            ...
            .bodyTags(
                submission.getBodyTags() != null ? submission.getBodyTags() : new ArrayList<>())
