...
public class AppUserService {
  ...
  /** Get enhanced user profile by user ID Includes workout stats and social metrics */
  ...
  /** Get enhanced user profile by username Includes workout stats and social metrics */
  ...
  private UserProfileDTO buildEnhancedProfile(AppUser user, UUID viewingUserId) {
    ...
    long followersCount =
        followRepository.countByFolloweeAndStatus(user, Follow.FollowStatus.ACCEPTED);
    long followingCount =
        followRepository.countByFollowerAndStatus(user, Follow.FollowStatus.ACCEPTED);
  ...
  private WorkoutStatsDTO calculateWorkoutStats(AppUser user) {
    ...
    long workoutsThisWeek =
        workoutRepository.countByUserAndDatePerformedBetween(user, startOfWeek, endOfWeek);
  ...
  private Map<String, Integer> buildWeeklySplit(
      AppUser user, LocalDate startOfWeek, LocalDate endOfWeek) {
    List<Workout> weekWorkouts =
        workoutRepository.findByUserAndDatePerformedBetween(user, startOfWeek, endOfWeek);
  ...
  public List<UserDTO> searchUsersByUsername(String query) {
    return userRepository.searchByUsername(query).stream().map(this::convertToDTO).toList();
  }
}
