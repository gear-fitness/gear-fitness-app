    ...
    try {
      ...
      GenerateContentConfig config =
          GenerateContentConfig.builder()
              .maxOutputTokens(250)
              .systemInstruction(
                  Content.builder().parts(List.of(Part.fromText(systemText))).build())
              .build();

      // ✅ Pass history separately and add current message
      history.add(
          Content.builder().role("user").parts(List.of(Part.fromText(currentMessage))).build());

      GenerateContentResponse response =
          geminiClient.models.generateContent(modelName, history, config);
