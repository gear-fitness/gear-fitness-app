...
public class PersonalRecordService {
  ...
  public List<PersonalRecordDTO> getBigThreePRs(UUID userId) {
    ...
    for (Object[] row : results) {
      ...
      PersonalRecordDTO pr = new PersonalRecordDTO(exerciseName, weight, reps, date, workoutName);
    ...
    result.add(
        bestPRs.getOrDefault(
            "Bench Press", new PersonalRecordDTO("Bench Press", BigDecimal.ZERO, 0, null, null)));
    result.add(
        bestPRs.getOrDefault(
            "Squat", new PersonalRecordDTO("Squat", BigDecimal.ZERO, 0, null, null)));
    result.add(
        bestPRs.getOrDefault(
            "Deadlift", new PersonalRecordDTO("Deadlift", BigDecimal.ZERO, 0, null, null)));
